...
export default function DashboardPage() {
  ...
  if (pageState === "loading") {
    return (
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-4 gap-2">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-20 rounded-lg" />)}</div>
  ...
  if (pageState === "empty") {
    return (
      <Card><CardContent className="flex flex-col items-center gap-4 py-14 text-center">
        ...
        <p className="text-xs text-muted-foreground">暂无监控配置，请先在设置中创建监控</p>
        <Button size="sm" onClick={() => { window.location.href = "/dashboard/settings"; }}>去设置</Button>
      </CardContent></Card>
    );
  }
  ...
  return (
    <div ref={containerRef} className="space-y-3">
      ...
      {/* Stats Row */}
      <div className="dp-item grid grid-cols-2 gap-2 sm:grid-cols-4">
        {[
              ...
              <div key={item.id} onClick={() => setSelectedId(item.id ?? null)}
                ...
                {item.id != null && (
                  <button onClick={(e) => { e.stopPropagation(); toggleSave(item.id!); }}
                    className="shrink-0 rounded p-1 text-muted-foreground transition-colors hover:bg-secondary">
          ...
          <div className="space-y-3 p-4">
            {selected ? (
              <>
                ...
                ) : <div className="py-6 text-center text-xs text-muted-foreground">暂无选题建议</div>}
              </>
            ) : <div className="py-8 text-center text-xs text-muted-foreground">选择一个热点查看详情</div>}
          </div>
        </div>
      </div></div>
      ...
      <div className="dp-item">
        <div className="rounded-lg border border-border bg-card shadow-card">
          ...
          <div className="p-4">
            {notifications.length > 0 ? (
              <div className="grid gap-2 sm:grid-cols-2">
                {notifications.map((item) => (
                  <div key={item.id} className="flex items-center gap-3 rounded-lg border border-border/40 bg-black/20 p-3 hover:bg-secondary/30">
            ...
            ) : <div className="py-6 text-center text-xs text-muted-foreground">暂无未读通知</div>}
          </div>
        </div>
      </div>
    </div>
