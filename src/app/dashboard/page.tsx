...
type WorkspaceTab = "signal" | "evidence" | "report";
...
export default function DashboardPage() {
  ...
  const runEventAction = async (kind: "summary" | "extract") => {
    if (selectedId == null) return;
    setAction(kind);
    try {
      if (kind === "summary") await postEventsIdIntelligenceSummaryRegenerate({ id: selectedId });
      else await postEventsIdIntelligenceExtract({ id: selectedId });
      await loadDetail(selectedId);
      toast.success(kind === "summary" ? "事件摘要已重新生成" : "事件实体与声明已重新提取");
    } catch (reason) { toast.error(reason instanceof Error ? reason.message : "操作失败"); }
    finally { setAction(undefined); }
  };

  const runReportAction = async (kind: "build" | "preview") => {
    if (relatedReport?.id == null) return;
    setAction(kind);
    try {
      if (kind === "build") await postReportsIdBuild({ id: relatedReport.id });
      else await postReportsIdPreview({ id: relatedReport.id });
      await loadWorkspace();
      toast.success(kind === "build" ? "报告构建任务已提交" : "报告预览已刷新");
    ...
    <div className="grid min-h-[calc(100vh-60px)] grid-cols-1 xl:grid-cols-[minmax(0,1fr)_460px]">
      <section className="min-w-0 border-r border-border px-5 py-7 lg:px-11">
        ...
        <div className="flex gap-7 border-b border-border">
          {([['signal','信号'],['evidence','证据'],['report','报告']] as const).map(([value, label]) => <button key={value} onClick={() => setTab(value)} className={`relative pb-3 text-sm ${tab === value ? "text-blue-400" : "text-muted-foreground hover:text-foreground"}`}>{label}{tab === value && <span className="absolute inset-x-0 bottom-0 h-0.5 bg-blue-500" />}</button>)}
        </div>

        {tab === "evidence" && <div className="grid gap-6 py-6 lg:grid-cols-[minmax(0,1fr)_240px]">
      ...
      <aside className="bg-[#030303] px-5 py-7 lg:px-7">
        ...
        <div className="border-t border-border py-6"><p className="text-xs font-medium">情报任务</p><div className="mt-3 grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => runEventAction("extract")} disabled={!!action} className="gap-2 text-xs">{action === "extract" ? <Loader2 className="animate-spin" /> : <BookOpen />}提取实体</Button><Button onClick={() => runEventAction("summary")} disabled={!!action} className="gap-2 text-xs">{action === "summary" ? <Loader2 className="animate-spin" /> : <Sparkles />}重新生成摘要</Button></div></div>
        <div className="border-t border-border py-6"><div className="flex items-center justify-between"><p className="text-xs font-medium">关联报告</p><a href="/dashboard/reports" className="text-[11px] text-blue-400">查看全部</a></div>{relatedReport ? <div className="panel mt-3 p-4"><div className="flex items-center gap-2"><FileText className="h-4 w-4 text-muted-foreground" /><p className="truncate text-sm font-medium">{relatedReport.title || `报告 #${relatedReport.id}`}</p></div><p className="mt-2 line-clamp-3 text-xs leading-5 text-muted-foreground">{relatedReport.summary || "该报告尚未生成摘要。"}</p><div className="mt-3 flex items-center justify-between text-[11px] text-muted-foreground"><span>{relatedReport.type}</span><span>{relatedReport.status}</span></div><div className="mt-4 grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => runReportAction("preview")} disabled={!!action} className="text-xs">预览</Button><Button onClick={() => runReportAction("build")} disabled={!!action} className="text-xs">构建报告</Button></div></div> : <div className="panel mt-3 p-5 text-xs text-muted-foreground">后端尚未生成报告记录。</div>}</div>
